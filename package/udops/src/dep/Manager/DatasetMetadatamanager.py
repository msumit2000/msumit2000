...
class DatasetMetadatamanager:
    ...
    def corpus_custom_fields(self ,datasetname,kv_pairs):
        conn = connection.get_connection()
        cur = conn.cursor()
...
############## function for dataset api#########
    ...
    def search_dataset(self,property):
        try:
            conn = connection.get_connection()
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            query = f"select dataset_id, dataset_name,corpus_filter from dataset_metadata where corpus_type='{property}';"
            cursor.execute(query)
            rows = cursor.fetchall()
            conn.commit()
            cursor.close()
    ...
    def dataset_corpus_list(self,dataset_id):
        try:
            conn = connection.get_connection()
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            query = f"select dataset_corpus_list.corpus_name from dataset_corpus_list join dataset_metadata on dataset_corpus_list.dataset_id= dataset_metadata.dataset_id where dataset_metadata.dataset_id ={dataset_id};"
            cursor.execute(query)
            rows = cursor.fetchall()
            conn.commit()
            cursor.close()
